        ...
        <div class="rs_content">
        	...
        	<input type="hidden" name="id" id="id">
	            ...
	            <div class="address_content">
					 <span class="red">*</span><span class="kuan">省&nbsp;&nbsp;份：</span><select data-province="---- 选择省 ----" name="receiverState"id="receiverState" onclick="getCity(this.value,-1,-1);"></select>
					  城市：<select data-city="---- 选择市 ----" name="receiverCity" id="receiverCity" onclick="getArea(this.value,-1)"></select>
			...
			success:function(obj){
                ...
                for(i=0;i<obj.data.length;i++){
                	if(obj.data[i].isDefault==1){
                        ...
                        '<span class="dzxq dzxq_normal">'+obj.data[i].recvDistric+'</span>'+
                        '<span class="lxdh lxdh_normal">'+obj.data[i].recvPhone+'</span>'+
                        '<span class="operation operation_normal">'+
                        	'<span class="aco_change" onclick="goUpdate('+obj.data[i].id+')">修改</span>|<span class="aco_delete"onclick="goDelete('+obj.data[i].id+')">删除</span>'+
                        '</span>'+
	...
	function getProvince(provinceCode,cityCode,areaCode){
		$.ajax({
			...
			success:function(obj){
				...
				for(i=0;i<obj.data.length;i++){
					var op= new Option(obj.data[i].provinceName,obj.data[i].provinceCode);
	...
	function getCity(provinceCode,cityCode,areaCode){
		$.ajax({
			...
			success:function(obj){
				...
				for(i=0;i<obj.data.length;i++){
					var op=new Option(obj.data[i].cityName,obj.data[i].cityCode);
	...
	function getArea(cityCode,areaCode){
		$.ajax({
			...
			success:function(obj){
				...
				for(i=0;i<obj.data.length;i++){
					var op=new Option(obj.data[i].areaName,obj.data[i].areaCode);
